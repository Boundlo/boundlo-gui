...
const SBFileUploaderHOC = function (WrappedComponent) {
    class SBFileUploaderComponent extends React.Component {
        ...
        createFileObjects () {
            ...
            this.inputElement.accept = '.sb,.sb2,.sb3,.pmp,.bl1exp,.bnd1exp';
        ...
        getProjectTitleFromFilename (fileInputFilename) {
            ...
            // (.sb, .sb2, .sb3, .pmp, .bl1exp, and finally, .bnd1exp)
            const matches = fileInputFilename.match(/^(.*)\.(sb[23]?|pmp|bl1exp|bnd1exp)$/);
            if (!matches) return '';
